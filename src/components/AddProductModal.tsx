  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await productService.addProduct({
        ...
        sellerId: user.uid,
        categoryId: 'general', // Default for MVP
      });
      onSuccess();
      onClose();
    } catch (err) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: '1.5rem', marginBottom: '2rem', textAlign: 'center' }}>Add New Product</h2>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
            <button type="button" onClick={onClose} style={{ flex: 1, background: 'none', border: '1px solid var(--border)', color: 'var(--text-main)', padding: '0.75rem', borderRadius: '8px', cursor: 'pointer' }}>Cancel</button>
            <button type="submit" disabled={loading} className="btn-primary" style={{ flex: 2 }}>{loading ? 'Adding...' : 'Add Product'}</button>
          </div>
        </form>
      </div>
    </div>
